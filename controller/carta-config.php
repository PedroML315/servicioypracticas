...
// TODO: protege este endpoint (ej. solo admin):
...
switch ($_SERVER['REQUEST_METHOD']) {
  ...
  case 'POST': {
    ...
    if (!hash_equals($_SESSION['csrf_token'] ?? '', $token)) {
      ...
      echo json_encode(['ok' => false, 'error' => 'CSRF inválido']); break;
    }

    $ctype = $_SERVER['CONTENT_TYPE'] ?? '';
    if (stripos($ctype, 'application/json') === false) {
      http_response_code(415);
      echo json_encode(['ok' => false, 'error' => 'Content-Type debe ser application/json']); break;
    }
    ...
    if (!is_array($data)) {
      http_response_code(400);
      echo json_encode(['ok' => false, 'error' => 'JSON inválido']); break;
    }

    // Normalización mínima (opcional: validar esquema)
    $cfg = $data;

    if (!saveConfig($CONFIG_PATH, $cfg)) {
      http_response_code(500);
      echo json_encode(['ok' => false, 'error' => 'No se pudo guardar el JSON']); break;
    }
