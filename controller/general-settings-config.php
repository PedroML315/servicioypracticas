<?php
// controller/general-settings-config.php
...
$CONFIG_PATH = __DIR__ . '/../config/general_settings.json';
...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST': {
        ...
        // Validar email_capacitacion
        $email = trim($data['email_capacitacion'] ?? '');
        if ($email !== '' && !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            http_response_code(422);
            echo json_encode(['ok' => false, 'error' => 'El correo electrónico no es válido'], JSON_UNESCAPED_UNICODE);
            break;
        }

        $current = loadConfig($CONFIG_PATH);
        $current['email_capacitacion'] = $email;

        if (saveConfig($CONFIG_PATH, $current)) {
            echo json_encode(['ok' => true, 'message' => 'Configuración guardada correctamente'], JSON_UNESCAPED_UNICODE);
        } else {
            http_response_code(500);
            echo json_encode(['ok' => false, 'error' => 'No se pudo guardar la configuración'], JSON_UNESCAPED_UNICODE);
        }
        break;
    }
