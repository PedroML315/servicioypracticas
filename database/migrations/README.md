...
| 21 | `create_directory_notificacion_vacante.php` | Tabla `directory` (directorio institucional: directores/vicerrectores) + plantilla `directorio_vacante_aprobada`. Al aceptar una solicitud de practicantes se envía un correo con los datos de la vacante a todos los contactos del directorio. |
| 22 | `create_internship_reminders.php` | Recordatorios automáticos de prácticas: tabla `internship_reminders` + 10 plantillas `pp_recordatorio_*` (5 momentos × alumno/empresa). Los envía `controller/cron/cron_recordatorios_practicas.php`, que requiere una tarea programada diaria a las 12:00. Siembra los umbrales ya rebasados para no mandar avisos retroactivos. |
